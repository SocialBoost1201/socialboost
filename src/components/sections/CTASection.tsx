      ...
      <Container className="relative z-10 text-center">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link href="/contact" tabIndex={-1} className="w-full sm:w-auto">
            <Button size="lg" className="w-full sm:w-auto bg-white text-brand-primary hover:bg-gray-50 flex items-center gap-2 group shadow-xl transition-transform hover:-translate-y-1">
              <MessageCircle className="w-5 h-5" />
              まずは無料で相談してみる
              <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
            </Button>
          </Link>
          <Link href="/works" tabIndex={-1} className="w-full sm:w-auto mt-4 sm:mt-0">
            <Button size="lg" variant="ghost" className="w-full sm:w-auto text-white hover:bg-white/10 hover:text-white border border-white/30 transition-transform hover:-translate-y-1">
              制作実績を見る
            </Button>
          </Link>
        </div>
